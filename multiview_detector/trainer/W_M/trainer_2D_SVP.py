...
class PerspectiveTrainer_2D_SVP(BaseTrainer):
    ...
    def train(self, epoch, data_loader, optimizer, log_interval=100, cyclic_scheduler=None):
        ...
        for batch_idx, (data, map_gt, img_gt, _) in enumerate(data_loader):
            optimizer.zero_grad()
            img_res, view_gp_res, view_mask = self.model(data)
            t_f = time.time()
            t_forward += t_f - t_b
            # 2D
            gaussian_img_gt = target_transform(img_res, img_gt[0], data_loader.dataset.img_kernel)
            loss_2D = F.mse_loss(img_res, gaussian_img_gt.to(img_res.device))
            ...
            if (batch_idx + 1) % log_interval == 0:
                ...
                print(
                    f'Train Epoch: {epoch}, Batch:{(batch_idx + 1)}, Loss: {losses / (batch_idx + 1):.6f}, loss_2D:{loss_2D:.6f}, loss_svp:{loss_SVP:.6f} '
                    f'lr:{current_lr:.6f}_'
                    f'Time: {t_epoch:.1f} (f{t_forward / batch_idx:.3f}+b{t_backward / batch_idx:.3f}), maxima:{view_gp_res.max():.3f}')
    ...
    def test(self, data_loader, epoch_resdir, visualize=True):
        ...
        losses /= len(data_loader)
        print(
            f'Test time:{t1 - t0:.1f}, losses:{losses:.6f}')
        return losses
