...
class PerspectiveTrainer(BaseTrainer):
    ...
    def train(self, data_loader, epoch, optimizer, log_interval=100, cyclic_scheduler=None, writer=None):
        ...
        for batch_idx, (imgs, img_gt, masked_view_gp_gt, gp_gt, frame) in enumerate(data_loader):
            ...
            t_forward += t_f - t_b
            # loss 2D
            loss_2D = F.mse_loss(img_res, img_gt.to(img_res.device).float())
            ...
            if (batch_idx + 1) % log_interval == 0:
                ...
                print(
                    f'Train Epoch: {epoch}, Batch:{batch_idx + 1}, Loss: {losses / (batch_idx + 1):.6f}, lr:{current_lr:.8f} '
    ...
    def test(self, data_loader, visualize=True, epoch_resdir=None, writer=None, first_test=False):
        self.model.eval()
        losses = 0
        t0 = time.time()
        for batch_idx, (imgs, img_gt, masked_view_gp_gt, gp_gt, frame) in enumerate(data_loader):
