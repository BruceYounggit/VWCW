...
class PerspectiveTrainer(BaseTrainer):
    ...
    def train(self, data_loader, epoch, optimizer, log_interval=100, cyclic_scheduler=None):
        self.model.train()
        losses = 0
        ...
        for batch_idx, (imgs, img_gt, masked_view_gp_gt, gp_gt, frame) in enumerate(data_loader):
            ...
            #     # Initialize weight
            #     weight_mask = torch.ones(3, 1, 384, 320) * 1 / 3
            ...
            img_res, view_gp_output, gp_res = self.model(imgs, gp_gt)
            t_f = time.time()
            t_forward += t_f - t_b
            ...
            if visualize and batch_idx % 100 == 0 and epoch_resdir:
                ...
                for view in range(0, data_loader.dataset.num_cam):
                    ...
                    if self.fix_weight == 0 or first_test:
                        plt.imshow(self.model.weight[view].detach().squeeze().cpu().numpy())
                        plt.colorbar()
                        plt.savefig(os.path.join(epoch_resdir, f'weight_frame{frame}_view{view}.jpg'))
                        plt.close()
    ...
    def counting_test(self, data_loader, gt_fpath=None, visualize=False, epoch_resdir=None, first_test=False):
        ...
        for batch_idx, (imgs, img_gt, masked_view_gp_gt, gp_gt, frame) in enumerate(data_loader):
            ...
            with torch.no_grad():
                img_res, view_gp_output, gp_res, weight = self.model(imgs)
                map_grid_res = gp_res.detach().cpu().squeeze()
                v_s = map_grid_res[map_grid_res > self.cls_thres].unsqueeze(1)
                grid_ij = (map_grid_res > self.cls_thres).nonzero()
            ...
            if batch_idx % 20:
                print(f'gp_gt sum={gp_gt.sum():.1f}')
            ...
            if visualize and batch_idx % 100 == 0 and epoch_resdir:
                ...
                for view in range(0, data_loader.dataset.num_cam):
                    ...
                    # w_mask vis
                    if self.fix_weight == 0 or first_test:
                        plt.imshow(weight[view].detach().squeeze().cpu().numpy())
                        plt.colorbar()
                        plt.savefig(os.path.join(epoch_resdir, f'weight_frame{frame}_view{view}.jpg'))
                        plt.close()
