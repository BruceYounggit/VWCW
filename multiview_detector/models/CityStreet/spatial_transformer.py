...
class SpatialTransformer_v3:
    def __init__(self,
        ...
        for ph in self.person_heights:
            if not os.path.exists(f'/home/yunfei/Study/Baseline_MVDet/Projection_files/Single_view_height'
                                  f'_projeciton_relation/proj_viewview2_height{int(ph)}_reso{self.output_size}.npy'):
            ...
            for view in range(1, 4):
                self.proj_views_heights[(view, int(ph))] = torch.from_numpy(np.load(
                    '/home/yunfei/Study/Baseline_MVDet/Projection_files/Single_view_height_projeciton_relation' +
                    f'/proj_viewview{view}_height{int(ph)}_reso{str(self.output_size)}.npy')).to(self.devicenum)
                # pass
        if not os.path.exists(f'/home/yunfei/Study/Baseline_MVDet/Projection_files/Single_view_height'
        ...
        for view in range(1, 4):
            self.view_gp_masks.append(torch.from_numpy(np.load('/home/yunfei/Study/Baseline_MVDet/Projection_files/'
                                                               'Single_view_height_projeciton_relation' +
                                                               f'/proj_viewview{view}_mask_outsize{output_size}.npy')).to(self.devicenum))
...
if __name__ == '__main__':
    ...
    res_dir = '/home/yunfei/Study/Multi_model_test_results/Img_results'
