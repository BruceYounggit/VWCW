...
class DPerspTransDetector(nn.Module):
    def __init__(self, dataset, arch='resnet18', **kwargs):
        ...
        # SVP
        self.view_gp_decoder = nn.Sequential(
            nn.Conv2d(out_channel * len(self.person_heights), 512, 3, padding=1),
            nn.ReLU(),
            nn.Conv2d(512, 512, 3, padding=1), nn.ReLU(),
            nn.Conv2d(512, 512, 3, padding=1), nn.ReLU(),
            nn.Conv2d(512, 256, 3, padding=1), nn.ReLU(),
            ...
            nn.Conv2d(64, 1, 1, bias=False)).to(self.device[1])
        # GP decoder
        self.GP_Decoder = nn.Sequential(
            nn.Conv2d(out_channel * len(self.person_heights), 512, 3, padding=1), nn.ReLU(),
            nn.Conv2d(512, 512, 3, padding=1), nn.ReLU(),
            nn.Conv2d(512, 512, 3, padding=1), nn.ReLU(),
            nn.Conv2d(512, 256, 3, padding=1), nn.ReLU(),
            nn.Conv2d(256, 128, 3, padding=1), nn.ReLU(),
        ...
        Initialize_net(self.GP_Decoder, mode='hkm')
    ...
    def forward(self, imgs, select_fusion='vcw'):
        ...
        # world_features = world_features.reshape(1, -1, *self.hgwg)
...
if __name__ == '__main__':
    ...
    img_res, view_gp_output, map_res, weight = model(imgs, 'paper')
    t4 = time.time()
