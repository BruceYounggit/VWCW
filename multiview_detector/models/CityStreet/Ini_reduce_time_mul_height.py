...
import numpy as np
import torch
from multiview_detector.datasets.CityStreet import camera_proj_Zhang as proj
...
def proj_2Dto2D(input_shape, out_shape, devicenum, person_heights):
    w = 676
    h = 380
    H = out_shape[0]
    W = out_shape[1]
    ...
    for view in range(1, 4):
        ...
        for ph in current_heights:
            ...
            np.save(os.path.join(
                '/home/yunfei/Study/Baseline_MVDet/Projection_files/Single_view_height_projeciton_relation',
                f'proj_view{view}_height{int(ph)}_reso{str(out_shape)}.npy'), view_ic)
    return proj_views_heights

def generate_cor_mask(out_shape):
    # view_gp_masks=[]
    for view in range(1, 4):
        # view_gp_masks
        if view == 1:
        ...
        # view_gp_mask = view_gp_mask.repeat(1, 1, 1, num_channels)
        # view_gp_masks.append(view_gp_mask)
        np.save(f'/home/yunfei/Study/Baseline_MVDet/Projection_files/Single_view_height_projeciton_relation/'
                f'proj_view{view}_mask_outsize{out_shape}.npy', view_gp_mask.numpy())
def test():
