...
from multiview_detector.models.CityStreet.Perspective_depthmap_2D_SVP_VCW_Multi_height_MVDet import \
    DPerspTransDetector as MVDet
from multiview_detector.models.CityStreet.Perspective_depthmap_2D_SVP_VCW_Multi_height_3DROM import \
    DPerspTransDetector as TDROM
# Pespective_Trainer
from multiview_detector.trainer.CityStreet.trainer_depthmap_2D_SVP_VCW_Multi_height import PerspectiveTrainer
...
def model_run(args):
    ...
    if args.multiheight == 'mvdet':
        model = MVDet(train_set, arch=args.arch, fix_2D=args.fix_2D,
                      fix_svp=args.fix_svp, fix_weight=args.fix_weight, person_heights=args.person_heights)
    elif args.multiheight == '3drom':
        model = TDROM(train_set, arch=args.arch, fix_2D=args.fix_2D,
                      fix_svp=args.fix_svp, fix_weight=args.fix_weight, person_heights=args.person_heights)
    else:
    ...
    if 'resnet' in args.arch:
        optimizer1 = optim.SGD([{'params': model.base_pt1.parameters(), 'lr': args.lr},
                                ...
                                # {'params': model.depth_classifier.parameters(), 'lr': args.lr}
    ...
    scheduler2 = optim.lr_scheduler.OneCycleLR(optimizer2, max_lr=args.lr * args.lrfac,
                                               steps_per_epoch=len(train_loader),
                                               epochs=args.epochs)
    ...
    logdir = f'logs/{args.dataset}_frame/{args.arch}/{args.variant}/' + datetime.datetime.today().strftime(
        '%Y-%m-%d_%H-%M-%S') + f'heights{args.person_heights}_fix2D{args.fix_2D}w{args.weight_2D}_fixsvp{args.fix_svp}w{args.weight_svp}_fixconf{args.fix_weight}_' \
    ...
    if args.resume is None:
        ...
        if args.multiheight == '3drom':
            shutil.copy('/home/yunfei/Study/MVD_VCW/multiview_detector/models/CityStreet/'
                        'Perspective_depthmap_2D_SVP_VCW_Multi_height_MVDet.py',
                        logdir + '/scripts/Perspective_depthmap_2D_SVP_VCW_Multi_height_3DROM.py')
        elif args.multiheight == 'shot':
            shutil.copy('/home/yunfei/Study/MVD_VCW/multiview_detector/models/CityStreet/'
                        'Perspective_depthmap_2D_SVP_VCW_Multi_height_MVDet.py',
                        logdir + '/scripts/Perspective_depthmap_2D_SVP_VCW_Multi_height_SHOT.py')
        shutil.copy(
            '/home/yunfei/Study/MVD_VCW/multiview_detector/trainer/CityStreet/trainer_depthmap_2D_SVP_VCW_Multi_height.py',
            logdir + '/scripts/trainer_depthmap_2D_SVP_VCW_Multi_height.py')
        shutil.copy(
            '/home/yunfei/Study/MVD_VCW/multiview_detector/x_training/CityStreet/model_run_2D_SVP_VCW_Multi_height.py',
            logdir + '/scripts/model_run_2D_SVP_VCW_Multi_height.py')
    ...
    if args.resume is None:
        ...
        if args.arch == 'resnet18':
            ...
            # pretrained_model_dir = '/home/yunfei/Study/Baseline_MVDet/logs/citystreet_frame' \
            #                        '/2D/2022-11-10_00-21-45/MultiviewDetector_50.pth'
            # pretrained_model_dir='/home/yunfei/Study/MVD_VCW/logs/citystreet_frame/vgg16/2D_SVP_VCW/2023-05-23_15-56-08_fix2D1.0w1_' \
            #                      'fixsvp0.0w1_fixconf0.0_momentum0.9_weight_decay0.0001_lr0.01_lrsonecycle_epo200_' \
            #                      'ct0.4_nt10_dt20/MultiviewDetector.pth'
            # pretrained_model_dir = '/home/yunfei/Study/MVD_VCW/logs/citystreet_frame/vgg16/2D_SVP_VCW/2023-05-23_15-56-08' \
            #                        '_fix2D1.0w1_fixsvp0.0w1_fixconf0.0_momentum0.9_weight_decay0.0001_lr0.01' \
            #                        '_lrsonecycle_epo200_ct0.4_nt10_dt20/MultiviewDetector.pth'
            pretrained_model_dir='/home/yunfei/Study/MVD_VCW/logs/citystreet_frame/vgg16/MH_2D_SVP_VCW/2023-06-06_15-59-07heights[0, 500, 1000, 1500, 2000]_fix2D1.0w1_fixsvp1.0w1_fixconf0.0_momentum0.9_weight_decay0.0001_lr0.01_lrsonecycle_epo20_ct0.4_nt10_dt20/MultiviewDetector.pth'
        pretrained_model= torch.load(pretrained_model_dir, map_location='cuda:0')
        model_dict = model.state_dict()
        pretrained_dict = pretrained_model['net']
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and 'confidence' not in k}
        ...
        # trainer.counting_test(test_loader, train_set.gt_fpath, True, epoch_dir, first_test=True)
        print('\n')
